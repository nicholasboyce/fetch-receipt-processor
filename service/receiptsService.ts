...
const processRetailer = (retailer: string): number => {
  ...
  for (let i = 0; i < retailer.length; i++) {
    const char = retailer.charCodeAt(i);
    const alphanum =
      (char > 47 && char < 58) || // is it 0-9?
...
const saveReceipt = async (receipt: validatedReceipt, points: number) => {
  ...
  const parsedReceiptResult = receiptDBSchema.safeParse({
    id,
    ...receipt,
    points,
  }); // make sure obj spreading successful
  ...
  const savedReceiptID = await receiptsRepository.saveReceipt(
    parsedReceiptResult.data,
  );
  if (!savedReceiptID) {
    return null;
  }
